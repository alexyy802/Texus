...
class PlayerManager:
    ...
    def __init__(self, audio, player):
    ...
    def create(self, guild_id: int, region: str = 'eu', endpoint: str = None, node: Node = None):
        ...
        If endpoint is provided, audio will attempt to parse the region from the endpoint
        ...
        audio will fall back to the node with the lowest penalty.
        ...
        if endpoint:  # Prioritise endpoint over region parameter
            region = self._audio.node_manager.get_region(endpoint)

        best_node = node or self._audio.node_manager.find_ideal_node(region)
