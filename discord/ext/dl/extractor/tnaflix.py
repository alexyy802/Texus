...
class TNAFlixNetworkBaseIE(InfoExtractor):
        ...
        return [
            {
                "url": self._proto_relative_url(
                    pattern_el.text.replace("#", compat_str(i)), "http:"
                ),
                "width": width,
                "height": height,
            }
    ...
    def _real_extract(self, url):
        ...
        for item in cfg_xml.findall("./quality/item"):
            ...
            height = int_or_none(
                self._search_regex(r"^(\d+)[pP]", format_id, "height", default=None)
            )
            formats.append(
                {
                    "url": self._proto_relative_url(
        ...
        duration = parse_duration(
            self._html_search_meta("duration", webpage, "duration", default=None)
        )
...
class TNAFlixIE(TNAEMPFlixBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?tnaflix\.com/[^/]+/(?P<display_id>[^/]+)/video(?P<id>\d+)"
    )
