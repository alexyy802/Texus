        ...
        if video_url_base:
            ...
            formats.extend(
                self._extract_ism_formats(
                    video_url_base + ".ism/Manifest", video_id, "mss", fatal=False
                )
            )
            formats.extend(
            ...
            m3u8_formats = list(
                filter(lambda f: f.get("vcodec") != "none", m3u8_formats)
            )
    ...
    def _entries(self, display_id, playlist_id):
        ...
        for page_num in itertools.count(1):
            ...
            video_ids = orderedSet(
                re.findall(
                    r'<a[^>]+\bhref=["\']/video/%s,[^,]+,(\d+)' % display_id, page
                )
            )
            ...
            for video_id in video_ids:
                yield self.url_result(
                    "tvp:%s" % video_id, ie=TVPEmbedIE.ie_key(), video_id=video_id
                )

    def _real_extract(self, url):
        mobj = re.match(self._VALID_URL, url)
        display_id, playlist_id = mobj.group("display_id", "id")
        return self.playlist_result(self._entries(display_id, playlist_id), playlist_id)
