...
class PinkbikeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        uploader = self._search_regex(
            r'<a[^>]+\brel=["\']author[^>]+>([^<]+)', webpage, "uploader", fatal=False
        )
        ...
        location = self._html_search_regex(
            r"(?s)<dt>Location</dt>\s*<dd>(.+?)<", webpage, "location", fatal=False
        )

        def extract_count(webpage, label):
            return str_to_int(
                self._search_regex(
                    r'<span[^>]+class="stat-num"[^>]*>([\d,.]+)</span>\s*<span[^>]+class="stat-label"[^>]*>%s'
                    % label,
                    webpage,
                    label,
                    fatal=False,
