...
class TagesschauPlayerIE(InfoExtractor):
    ...
    def _extract_via_api(self, kind, video_id):
        info = self._download_json(
            "https://www.tagesschau.de/api/multimedia/{0}/{0}-{1}.json".format(
                kind, video_id
            ),
            video_id,
        )
...
class TagesschauIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return (
            False
            if TagesschauPlayerIE.suitable(url)
            else super(TagesschauIE, cls).suitable(url)
        )
    ...
    def _real_extract(self, url):
        ...
        description = self._html_search_regex(
            r'(?s)<p class="teasertext">(.*?)</p>', webpage, "description", default=None
        )
