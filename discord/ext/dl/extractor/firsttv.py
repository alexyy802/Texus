...
class FirstTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        playlist_url = compat_urlparse.urljoin(
            url,
            self._search_regex(
        ...
        for item in items:
            ...
            for f in item.get("mbr", []):
                ...
                if not src:
                    continue
                tbr = int_or_none(
                    self._search_regex(r"_(\d{3,})\.mp4", src, "tbr", default=None)
                )
                if not path:
                    path = self._search_regex(
                        r"//[^/]+/(.+?)_\d+\.mp4", src, "m3u8 path", default=None
                    )
                formats.append(
                    {
                        "url": src,
            ...
            if not path and len(formats) == 1:
                path = self._search_regex(
                    r"//[^/]+/(.+?$)", formats[0]["url"], "m3u8 path", default=None
                )
            if path:
                if len(formats) == 1:
                    m3u8_path = ","
