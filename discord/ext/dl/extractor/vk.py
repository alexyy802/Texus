...
class VKIE(VKBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if m_rutube is not None:
            rutube_url = self._proto_relative_url(m_rutube.group(1).replace("\\", ""))
...
class VKUserVideosIE(VKBaseIE):
    ...
    def _real_extract(self, url):
        ...
        entries = OnDemandPagedList(
            functools.partial(self._fetch_page, page_id, section), self._PAGE_SIZE
        )
            ...
            entries.append(
                {
                    ...
                    "thumbnails": [{"url": c_url} for c_url in a.cover_url.split(",")]
                    if a.cover_url
                    else None,
